...
if (!string.IsNullOrEmpty(connectionString))
{
    ...
    builder.Services.AddOpenTelemetry()
        ...
        .WithTracing(tracing =>
        {
            ...
            // tracing.AddEntityFrameworkCoreInstrumentation(); // Método no disponible
        })
        .WithMetrics(metrics =>
        {
            metrics.AddAspNetCoreInstrumentation();
            metrics.AddHttpClientInstrumentation();
            // metrics.AddRuntimeInstrumentation(); // Método no disponible
        })
        .WithLogging(logging =>
        {
            // logging.AddApplicationInsightsConsole(); // Método no disponible
        });
...
{
    cfg.RegisterServicesFromAssembly(typeof(SecureBank.Application.Features.Authentication.Commands.RegisterUser.RegisterUserCommand).Assembly);
});

// FluentValidation
builder.Services.AddValidatorsFromAssembly(typeof(SecureBank.Application.Features.Authentication.Commands.RegisterUser.RegisterUserCommand).Assembly);
...
builder.Services.AddScoped<Application.Common.Interfaces.IAzureMonitorService, Infrastructure.Services.AzureMonitorService>();
...
.AddJwtBearer(options =>
{
    ...
    options.Events = new JwtBearerEvents
    {
        OnTokenValidated = async context =>
        {
            ...
            var azureMonitor = context.HttpContext.RequestServices.GetRequiredService<Application.Common.Interfaces.IAzureMonitorService>();
            
            // Log de validación de token exitosa
            await azureMonitor.LogSecurityEventAsync(new Application.Common.Interfaces.SecurityEvent 
            {
...
{
    options.Filters.Add<SecureBank.AuthAPI.Filters.SecurityAuditActionFilter>();
    options.Filters.Add<SecureBank.AuthAPI.Filters.ExceptionHandlingFilter>();
})
